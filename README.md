...
`@assetpulse/sdk` exists to remove all four:
...
```bash
npm install @assetpulse/sdk
# or
pnpm add @assetpulse/sdk
# or
yarn add @assetpulse/sdk
```
...
```ts
import { AssetPulseClient, AssetPulseApiError } from "@assetpulse/sdk";
...
`@assetpulse/sdk` was designed to sit inside Next.js Server Components, Server Actions, and Route Handlers — anywhere a session's JWT is available server-side. **Never instantiate the client in browser code with a raw token**; read it from an `httpOnly` session cookie on the server first.
...
npm link @assetpulse/sdk
```

Run `npm unlink @assetpulse/sdk` in the consuming app afterward to go back to the published version.
